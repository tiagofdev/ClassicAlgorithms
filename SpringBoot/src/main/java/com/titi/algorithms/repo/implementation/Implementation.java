...
public class Implementation implements Service {
    ...
    public ArrayList<String> knapsack(int[] values, int[] weights, int maximum) {
        ...
        for (int ix = 1; ix <= values.length; ix++) {
            ...
            for (int w = 0; w <= maximum; w++) {
                ...
                if (weights[ix-1] > w) {
                    ...
                    T[ix][w] = Integer.max(T[ix-1][w], T[ix-1][w-weights[ix-1]] + values[ix-1]);
                }
            }
        }
        ...
        for (int ix = values.length; ix > 0 && res > 0; ix--) {
            ...
            if (!(res == T[ix - 1][mw])) {
                ...
                wts.append(weights[ix - 1]).append(" ");
    ...
    public static void computeLPS(String pattern, int pSize, int[] lps) {
        ...
        lps[0] = 0;
        while (ix < pSize) {
            if (pattern.charAt(ix) == pattern.charAt(length)) {
                length++;
                lps[ix] = length;
                ix++;
            }
    ...
    public ArrayList<String> coinChange(int[] coins, int target) {
        ...
        if ( target > 100 )
            return results;
        findNumbers(answers, values, target, 0, temp);
        results.add(Integer.toString(answers.size()));
        results.add("");
        for (ArrayList<Integer> answer : answers) {
            StringBuilder combo = new StringBuilder();
            for (int ix : answer) {
                combo.append(ix).append(",");
            }
    ...
    public static void findNumbers(ArrayList<ArrayList<Integer>> answer,
        ...
        for (int ix = index; ix < coins.size(); ix++) {
            // checking that sum does not become negative
            if ((target - coins.get(ix)) >= 0) {
                // adding element which can contribute to sum
                temp.add(coins.get(ix));
                findNumbers(answer, coins, target - coins.get(ix), ix, temp);
                // removing element from list (backtracking)
                temp.remove(coins.get(ix));
            }
        }
    }
    ...
    public ArrayList<String> jobSequencing(String inputJobs, int deadline) {
        ...
        ArrayList<Job> sequencedJobs = new ArrayList<>();
        PriorityQueue<Job> maxProfitPriority = new PriorityQueue<>((a,b) -> b.profit - a.profit);
        for (int ix = jobs.size() - 1; ix > -1; ix--) {
            int slotAvailable;

            if (ix == 0) {
                slotAvailable = jobs.get(ix).deadline;
            }
            else {
                slotAvailable = jobs.get(ix).deadline - jobs.get(ix - 1).deadline;
            }
            maxProfitPriority.add(jobs.get(ix));
            while(slotAvailable > 0 && maxProfitPriority.size() > 0) {
                Job job = maxProfitPriority.remove();
                slotAvailable--;
                sequencedJobs.add(job);
                totalProfit += job.profit;
            }
        }
        sequencedJobs.sort(Comparator.comparingInt(a -> a.deadline));
