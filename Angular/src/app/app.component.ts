...
export class AppComponent implements AfterViewInit {
  ...
  @ViewChild('onesieForm') private _myForm!: NgForm;
  ...
  switchComponent(problem: any): void {
    ...
    document.getElementById(this.currentComponent.button)!.style.backgroundColor = "#777F87";
    ...
    if (problem.value === "about") {
      ...
      this.component = new this.currentComponent.component(this.controller);
      // This data does not come from the server, it sits in each component on the client side
      this.data = this.component.getData();
    }
    const sigma_container = document.getElementById("sigma-container") as HTMLElement;
    if ( sigma_container != null ) {
  ...
  // This function tells components to get a response from Spring Boot server
  routeResponse(form: NgForm): void {
    ...
    if ( this.component.data.title === "Depth First Search" ||
      ...
      // Tell component to get result from Backend Server
      this.response$ = this.component.getResponse(form);
    }
